...
export function resolveNgBin(): string {
  throw new Error(
    'Dashboard no longer uses Angular CLI. Build the dashboard with "pnpm --filter @crucible/dashboard build" and rebuild the CLI so assets are staged under packages/cli/dashboard.',
  );
}
