...
import { gitRoot } from "../shared/git.js";
...
function validateConfig(path: string): void {
  ...
  const limit = (
    tracking as { max_snapshot_file_size_bytes?: unknown }
  ).max_snapshot_file_size_bytes;
  ...
  if (
    ...
    throw new Error(
      "INVALID_PROJECT_CONFIG: max_snapshot_file_size_bytes must be a positive integer.",
    );
  }
}


export function initializeProject(directory: string): InitializedProject {
  const root = gitRoot(directory);
  const crucibleDirectory = resolve(root, ".crucible");
  const projectPath = resolve(crucibleDirectory, "project.json");
  const configPath = resolve(crucibleDirectory, "config.yaml");

  if (
    existsSync(crucibleDirectory) &&
    lstatSync(crucibleDirectory).isSymbolicLink()
  ) {
    throw new Error(
      "INVALID_PROJECT_METADATA: .crucible must not be a symbolic link.",
    );
  }

  mkdirSync(crucibleDirectory, { recursive: true });
  ...
  if (!existsSync(projectPath)) {
    writeFileSync(
      projectPath,
      `${JSON.stringify({ project_id: id }, null, 2)}\n`,
