...
test("target matrix selects the expected optional package", () => {
  ...
  assert.deepEqual(
    ...
    { target: "linux-x64-gnu", packageName: "@pablozrrrr/crucible-core-linux-x64-gnu" },
  );
});
...
function makePackageFixture(options: FixtureOptions = {}): {
  ...
  const packageName = options.packageName ?? `@pablozrrrr/crucible-core-${target}`;
...
test("resolves a temp package and verifies the executable hash", () => {
  ...
  const resolved = resolveCore({
    ...
    requireResolve: (spec) => {
      assert.equal(spec, "@pablozrrrr/crucible-core-win32-x64/package.json");
      return fixture.packageJsonPath;
    },
  });

  assert.equal(resolved.executablePath, fixture.executablePath);
  assert.equal(resolved.packageName, "@pablozrrrr/crucible-core-win32-x64");
...
test("missing optional package names --omit=optional recovery", () => {
  ...
  assert.match(message, /optional package @pablozrrrr\/crucible-core-win32-x64/);
  assert.match(message, /--omit=optional/);
  assert.match(message, /npm install -g @pablozrrrr\/crucible-cli/);
});
