...
import { canonicalJson } from "../core-client.js";
import type {
  Admission,
...
// SQLite persistence for terminal obligations: schema, reservations, leases
// and the transactional ACK. Transport lives in terminal-delivery.ts and the
// lifecycle in terminal-outbox-controller.ts; this store never performs HTTP.
export class TerminalOutboxStore {
  ...
  static async open(policy: TerminalOutboxPolicy): Promise<TerminalOutboxStore> {
    const storage = await TerminalOutboxStore.connectStorage(policy);
    const outbox = new TerminalOutboxStore(policy, storage.database, storage.databasePath, storage.driverName);
    try {
  ...
  protected static async connectStorage(policy: TerminalOutboxPolicy): Promise<StorageConnection> {
    ...
    return { database: driver.open(databasePath), databasePath, driverName: driver.name };
  }
  ...
  releaseCandidate(id: string, diagnostic: string): void {
    this.database.exec("BEGIN IMMEDIATE");
    try {
      const row = this.database
  ...
  retryCandidate(row: CandidateRow, diagnostic: string): void {
    this.database.prepare(`
      UPDATE terminal_outbox SET attempts = attempts + 1, next_attempt_at = ?,
        last_diagnostic = ? WHERE id = ? AND state = 'candidate_in_flight'
    `).run(this.policy.clock() + this.backoffDelayMs(row.attempts), diagnostic, row.id);
  }
  ...
  finishDelivery(row: OutboxRow, delivery: OutboxDelivery): OutboxDelivery {
    this.database.exec("BEGIN IMMEDIATE");
    try {
      const result = this.database.prepare(`
        UPDATE terminal_outbox SET state = 'terminal', accounted_bytes = 0,
  ...
  retryDelivery(row: OutboxRow, diagnostic: string): OutboxDelivery {
    this.database.prepare(`
  ...
  leaseDelivery(id?: string): OutboxRow | undefined {
    const now = this.policy.clock();
    this.database.exec("BEGIN IMMEDIATE");
    try {
  ...
  listDueCandidates(): CandidateRow[] {
    const now = this.policy.clock();
    return this.database.prepare(`
      SELECT id, candidate_event_id, candidate_envelope, candidate_payload_hash,
             attempts, last_diagnostic
      FROM terminal_outbox WHERE state = 'candidate_in_flight' AND next_attempt_at <= ?
      ORDER BY created_at
    `).all(now) as CandidateRow[];
  }

  listDueDeliveryIds(): Array<{ id: string }> {
    const now = this.policy.clock();
    return this.database.prepare(`
